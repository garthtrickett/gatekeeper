...
fun executeDatabaseEffect(
    ...
    when (effect) {
        is GatekeeperEffect.DbLoadInitialState -> {
            ...
            val consumedCheckInsList = consumedCheckInsFromDb.map {
                com.aegisgatekeeper.app.domain.ConsumedCheckIn(it.id, it.groupId, it.timeMinutes.toInt(), it.timestamp)
            }

            val appGroupsList = groupsFromDb.map { group ->
                val appsForGroup = groupedApps.filter { it.groupId == group.id }.map { it.packageName }.toSet()
                val rulesForGroup = rulesFromDb.filter { it.groupId == group.id }.mapNotNull { rule ->
                    when (rule.ruleType) {
                        "TIME_LIMIT" -> timeLimits[rule.id]?.let { com.aegisgatekeeper.app.domain.BlockingRule.TimeLimit(rule.id, rule.groupId, rule.isEnabled, it.timeLimitMinutes.toInt()) }
                        "SCHEDULED" -> scheduledBlocks[rule.id]?.let { sb ->
                            com.aegisgatekeeper.app.domain.BlockingRule.ScheduledBlock(rule.id, rule.groupId, rule.isEnabled,
                                sb.timeSlots.split(",").filter { it.isNotEmpty() }.map { val parts = it.split("-"); com.aegisgatekeeper.app.domain.TimeSlot(parts[0].toInt(), parts[1].toInt()) },
                                sb.daysOfWeek.split(",").filter { it.isNotEmpty() }.map { com.aegisgatekeeper.app.domain.DayOfWeek.valueOf(it) }.toSet()
                            )
                        }
                        "CHECK_IN" -> checkInRules[rule.id]?.let { ci ->
                            com.aegisgatekeeper.app.domain.BlockingRule.CheckIn(rule.id, rule.groupId, rule.isEnabled,
                                ci.checkInTimes.split(",").filter { it.isNotEmpty() }.map { it.toInt() },
                                ci.durationMinutes.toInt(),
                                ci.daysOfWeek.split(",").filter { it.isNotEmpty() }.map { com.aegisgatekeeper.app.domain.DayOfWeek.valueOf(it) }.toSet()
                            )
                        }
                        "DOMAIN_BLOCK" -> domainBlocks[rule.id]?.let { dbBlock ->
                            com.aegisgatekeeper.app.domain.BlockingRule.DomainBlock(rule.id, rule.groupId, rule.isEnabled, dbBlock.domains.split(",").filter { it.isNotEmpty() }.toSet())
                        }
                        "ALWAYS_BLOCK" -> com.aegisgatekeeper.app.domain.BlockingRule.AlwaysBlock(rule.id, rule.groupId, rule.isEnabled)
                        else -> null
                    }
                }
            ...
            val safeChannels = try { db.safeYouTubeChannelQueries.selectAll().executeAsList().associate { it.channelId to it.channelName } } catch(e: Exception) { emptyMap() }
            ...
            val loadedState = com.aegisgatekeeper.app.domain.GatekeeperState(
                sync = com.aegisgatekeeper.app.domain.SyncAndIntegrationState(
                    ...
                    podcastSubscriptions = podcastSubscriptionsFromDb.map { com.aegisgatekeeper.app.domain.PodcastSubscription(it.id, it.feedUrl, it.showTitle, it.artworkUrl) },
                ),
                ...
                data = com.aegisgatekeeper.app.domain.DataState(
                    ...
                    contentItems = contentItemsFromDb.map { com.aegisgatekeeper.app.domain.ContentItem(it.id, it.podcastId, it.videoId, it.title, it.channelName, it.source, it.type, it.rank, it.capturedAtTimestamp, it.durationSeconds, it.lastModified, it.isSynced, it.isDeleted, it.localFilePath, it.downloadStatus) },
                    ...
                    intentionalSlots = slotsFromDb.map { slot -> com.aegisgatekeeper.app.domain.IntentionalSlotItem(slot.slotIndex.toInt(), com.aegisgatekeeper.app.domain.ContentItem(slot.id, slot.podcastId, slot.videoId, slot.title, slot.channelName, slot.source, slot.type, slot.rank, slot.capturedAtTimestamp, slot.durationSeconds, slot.lastModified, slot.isSynced, slot.isDeleted, slot.localFilePath, slot.downloadStatus)) },
                ),
                media = com.aegisgatekeeper.app.domain.MediaState(savedMediaPositions = mediaPositionsFromDb),
            )
            dispatch(com.aegisgatekeeper.app.domain.GatekeeperAction.InitialStateLoaded(loadedState))
        }
        is GatekeeperEffect.DbInsertVaultItem -> db.vaultItemQueries.insert(effect.item.id, effect.item.query, effect.item.capturedAtTimestamp, effect.item.isResolved, effect.item.lastModified, effect.item.isSynced, effect.item.isDeleted)
        is GatekeeperEffect.DbMarkVaultItemResolved -> db.vaultItemQueries.markAsResolved(effect.lastModified, effect.id)
        is GatekeeperEffect.DbInsertPodcastSubscription -> db.podcastSubscriptionQueries.insert(effect.subscription.id, effect.subscription.feedUrl, effect.subscription.showTitle, effect.subscription.artworkUrl)
        is GatekeeperEffect.DbUpsertContentItem -> db.contentItemQueries.insert(effect.item.id, effect.item.podcastId, effect.item.videoId, effect.item.title, effect.item.channelName, effect.item.source, effect.item.type, effect.item.rank, effect.item.capturedAtTimestamp, effect.item.durationSeconds, effect.item.lastModified, effect.item.isSynced, effect.item.isDeleted, effect.item.localFilePath, effect.item.downloadStatus)
        is GatekeeperEffect.DbUpdateDownloadStatus -> db.contentItemQueries.updateDownloadStatus(effect.status, effect.localFilePath, effect.lastModified, effect.id)
        is GatekeeperEffect.DbUpdateRank -> db.contentItemQueries.updateRank(effect.rank, effect.lastModified, effect.id)
        is GatekeeperEffect.DbRemoveFromContentBank -> db.transaction { db.contentItemQueries.delete(effect.lastModified, effect.id); db.intentionalSlotQueries.selectAll().executeAsList().forEach { if (it.id == effect.id) db.intentionalSlotQueries.delete(it.slotIndex) } }
        is GatekeeperEffect.DbCacheParsedEpisodes -> db.transaction { effect.episodes.forEachIndexed { i, ep -> db.podcastEpisodeQueries.insertOrReplace(randomUUIDString(), effect.podcastId, ep.title, ep.audioUrl, ep.durationSeconds, ep.pubDate, parseRssPubDate(ep.pubDate, 0L - i)) }; db.podcastEpisodeQueries.deleteOldEpisodes(effect.podcastId, 200) }
        ...
        is GatekeeperEffect.DbSetFrictionGame -> db.appSettingsQueries.updateFrictionGame(effect.game)
        is GatekeeperEffect.DbSetManualLockdown -> db.appSettingsQueries.updateManualLockdown(effect.isActive)
        is GatekeeperEffect.DbUpdatePhaseWindows -> db.appSettingsQueries.updatePhaseWindows(effect.deepWorkStartMinutes.toLong(), effect.deepWorkEndMinutes.toLong(), effect.gatheringStartMinutes.toLong(), effect.gatheringEndMinutes.toLong())
        is GatekeeperEffect.DbInsertNotificationDigest -> db.notificationDigestQueries.insert(effect.log.id, effect.log.packageName, effect.log.title, effect.log.content, effect.log.timestamp)
        is GatekeeperEffect.DbUpdateMissionControlApps -> db.transaction { db.missionControlAppQueries.deleteAll(); effect.packageNames.forEachIndexed { i, p -> db.missionControlAppQueries.insert(p, i.toLong()) } }
        is GatekeeperEffect.DbLogGiveUp -> db.giveUpLogQueries.insert(randomUUIDString(), effect.packageName, effect.timestamp)
        is GatekeeperEffect.DbInsertSessionLog -> db.sessionLogQueries.insert(effect.log.id, effect.log.packageName, effect.log.durationMillis, effect.log.emotion, effect.log.loggedAtTimestamp)
        is GatekeeperEffect.DbSetCustomInterceptionMessage -> db.customInterceptionMessageQueries.insert(effect.packageName, effect.message)
        is GatekeeperEffect.DbLogEmergencyBypass -> db.emergencyBypassLogQueries.insert(randomUUIDString(), effect.packageName, effect.reason, effect.timestamp)
        is GatekeeperEffect.DbSaveMediaPosition -> db.mediaPositionQueries.insert(effect.mediaId, effect.positionSeconds.toDouble())
        is GatekeeperEffect.DbAddPinnedWebsite -> db.missionControlWebsiteQueries.insert(effect.website.id, effect.website.label, effect.website.url, effect.rank)
        is GatekeeperEffect.DbRemovePinnedWebsite -> db.missionControlWebsiteQueries.delete(effect.id)
        is GatekeeperEffect.DbAddAlternativeActivity -> db.alternativeActivityQueries.insert(effect.activity.id, effect.activity.description, effect.activity.createdAtTimestamp)
        is GatekeeperEffect.DbRemoveAlternativeActivity -> db.alternativeActivityQueries.delete(effect.id)
        is GatekeeperEffect.DbInsertScheduledMessage -> db.scheduledMessageQueries.insert(effect.message.id, effect.message.beeperRoomId, effect.message.chatName, effect.message.messageText, effect.message.scheduledTimestamp, effect.message.status)
        is GatekeeperEffect.DbUpdateScheduledMessageStatus -> db.scheduledMessageQueries.updateStatus(effect.status, effect.id)
        is GatekeeperEffect.DbRemoteSyncCompleted -> db.transaction { effect.newVaultItems.forEach { db.vaultItemQueries.insert(it.id, it.query, it.capturedAtTimestamp, it.isResolved, it.lastModified, true, it.isDeleted) }; effect.newContentItems.forEach { db.contentItemQueries.insert(it.id, it.podcastId, it.videoId, it.title, it.channelName, it.source, it.type, it.rank, it.capturedAtTimestamp, it.durationSeconds, it.lastModified, true, it.isDeleted, it.localFilePath, it.downloadStatus) } }
        else -> {}
    }
}
