...
fun executeDatabaseEffect(
    ...
    when (effect) {
        is GatekeeperEffect.DbLoadInitialState -> {
            ...
            val timeLimits =
                    db.blockingRuleQueries.selectAllTimeLimitRules().executeAsList().associateBy {
                        it.ruleId
                    }
            val scheduledBlocks =
            ...
            val checkInRules =
                    db.blockingRuleQueries.selectAllCheckInRules().executeAsList().associateBy {
                        it.ruleId
                    }
            val domainBlocks =
                    db.domainBlockRuleQueries.selectAll().executeAsList().associateBy { it.ruleId }
            val consumedCheckInsFromDb =
                    db.blockingRuleQueries.selectAllConsumedCheckIns().executeAsList()
            val consumedCheckInsList =
                    consumedCheckInsFromDb.map {
                        com.aegisgatekeeper.app.domain.ConsumedCheckIn(
                                it.id,
                                it.groupId,
                                it.timeMinutes.toInt(),
                                it.timestamp
                        )
                    }
            ...
            val appGroupsList =
                    groupsFromDb.map { group ->
                        ...
                        com.aegisgatekeeper.app.domain.AppGroup(
                                ...
                                appsForGroup,
                                rulesForGroup,
                                group.ruleCombinator
                        )
                    }

            val customMessagesFromDb =
                    db.customInterceptionMessageQueries.selectAll().executeAsList().associate {
                        it.packageName to it.message
                    }
            val alternativeActivitiesFromDb =
                    db.alternativeActivityQueries.selectAll().executeAsList()
            ...
            val podcastSubscriptionsFromDb =
                    db.podcastSubscriptionQueries.selectAll().executeAsList()
            val mediaPositionsFromDb =
                    db.mediaPositionQueries.selectAll().executeAsList().associate {
                        it.mediaId to it.positionSeconds.toFloat()
                    }
            val safeChannels =
                    try {
                        db.safeYouTubeChannelQueries.selectAll().executeAsList().associate {
            ...
            val pinnedWebsitesFromDb =
                    db.missionControlWebsiteQueries.selectAll().executeAsList().map {
                        com.aegisgatekeeper.app.domain.PinnedWebsite(it.id, it.label, it.url)
                    }
                    ...
                    com.aegisgatekeeper.app.domain.GatekeeperState(
                            ...
                            interception =
                                    com.aegisgatekeeper.app.domain.InterceptionState(
                                            isManualLockdownActive =
                                                    appSettings?.isManualLockdownActive ?: false,
                                            activeFrictionGame = appSettings?.activeFrictionGame
                                                            ?: com.aegisgatekeeper.app.domain
                                                                    .FrictionGame.GAUNTLET,
                                            appGroups = appGroupsList,
                                    ),
                            data =
                                    com.aegisgatekeeper.app.domain.DataState(
                                            deepWorkStartMinutes =
                                                    appSettings?.deepWorkStartMinutes?.toInt()
                                                            ?: 540,
                                            deepWorkEndMinutes =
                                                    appSettings?.deepWorkEndMinutes?.toInt()
                                                            ?: 1020,
                                            gatheringStartMinutes =
                                                    appSettings?.gatheringStartMinutes?.toInt()
                                                            ?: 1080,
                                            gatheringEndMinutes =
                                                    appSettings?.gatheringEndMinutes?.toInt()
                                                            ?: 1110,
            ...
            dispatch(
                    com.aegisgatekeeper.app.domain.GatekeeperAction.InitialStateLoaded(loadedState),
            )
        }
        ...
        is GatekeeperEffect.DbAddAlwaysBlockRule -> {
            db.blockingRuleQueries.insertBlockingRule(
                    effect.id,
                    effect.groupId,
                    "ALWAYS_BLOCK",
                    true
        ...
        is GatekeeperEffect.DbAddTimeLimitRule -> {
            db.transaction {
                ...
                db.blockingRuleQueries.insertTimeLimitRule(
                        effect.id,
                        effect.timeLimitMinutes.toLong()
                )
            }
        }
        ...
        is GatekeeperEffect.DbLoadCachedPodcastEpisodes -> {
            val episodes =
                    db.podcastEpisodeQueries
                            .selectAllForPodcast(effect.podcastId)
                            .executeAsList()
                            .map {
            ...
            dispatch(
                    com.aegisgatekeeper.app.domain.GatekeeperAction.PodcastEpisodesLoaded(
                            episodes,
                            effect.podcastId
                    ),
            )
        ...
        is GatekeeperEffect.DbLoadLatestGlobalEpisodes -> {
            ...
            dispatch(
                    com.aegisgatekeeper.app.domain.GatekeeperAction.LatestGlobalEpisodesLoaded(
                            unifiedEpisodes
                    ),
            )
        }
        ...
        is GatekeeperEffect.DbUpdateMissionControlApps -> {
            db.transaction {
                ...
                effect.packageNames.forEachIndexed { i, p ->
                    db.missionControlAppQueries.insert(p, i.toLong())
                }
            }
        }
        ...
        is GatekeeperEffect.DbAddAlternativeActivity -> {
            db.alternativeActivityQueries.insert(
                    ...
                    effect.activity.createdAtTimestamp
            )
        }
