...
private fun reduceData(
    ...
    val newState = when (action) {
        ...
        is GatekeeperAction.ReorderContentBank -> {
            ...
            val newList = mutableList.mapIndexed { index, contentItem ->
        ...
        is GatekeeperAction.DownloadMediaRequested -> {
            ...
            slice.copy(contentItems = slice.contentItems.map { if (it.id == action.id) it.copy(downloadStatus = DownloadStatus.QUEUED) else it })
        }
        ...
        is GatekeeperAction.DownloadCompleted -> {
            ...
            slice.copy(contentItems = slice.contentItems.map { if (it.id == action.id) it.copy(downloadStatus = DownloadStatus.COMPLETED, localFilePath = action.localFilePath) else it })
        }
        is GatekeeperAction.DeleteDownloadedMedia -> {
            effects.add(GatekeeperEffect.DeleteDownloadedMedia(action.id))
            slice.copy(contentItems = slice.contentItems.map { if (it.id == action.id) it.copy(downloadStatus = DownloadStatus.NONE, localFilePath = null) else it })
        }
        is GatekeeperAction.RemovePodcastSubscription -> {
            ...
            slice.copy(
                contentItems = slice.contentItems.map { if (it.podcastId == podcastId) it.copy(isDeleted = true, lastModified = action.currentTimestamp) else it },
        ...
        is GatekeeperAction.RemoteSyncCompleted -> {
            ...
            action.newContentItems.forEach { remote ->
                ...
                if (current == null || current.lastModified < remote.lastModified) {
                    contentMap[remote.id] = if (current != null) {
        ...
        is GatekeeperAction.SaveIntentionalSlot -> {
            ...
            val newSlots = slice.intentionalSlots.filter { it.slotIndex != action.slotIndex } + IntentionalSlotItem(action.slotIndex, action.contentItem)
        ...
        is GatekeeperAction.RedeemCheckInToken -> {
            val log = ConsumedCheckIn(groupId = action.groupId, timeMinutes = action.checkInTimeMinutes, timestamp = action.currentTimestamp)
        ...
        is GatekeeperAction.LogSessionMetacognition -> {
            val log = SessionLog(packageName = action.packageName, durationMillis = action.durationMillis, emotion = action.emotion, loggedAtTimestamp = action.currentTimestamp)
...
private fun reduceMedia(
    ...
    val newState = when (action) {
        is GatekeeperAction.ProcessSharedLink -> {
            effects.add(GatekeeperEffect.FetchUrlMetadata(action.url, action.providedTitle, action.currentTimestamp, action.url.contains("soundcloud.com", ignoreCase = true), !action.url.contains("youtu.be", ignoreCase = true) && !action.url.contains("youtube.com", ignoreCase = true) && !action.url.contains("soundcloud.com", ignoreCase = true)))
        ...
        is GatekeeperAction.AddEpisodeToBank -> {
            ...
            effects.add(GatekeeperEffect.EmitAction(GatekeeperAction.SaveToContentBank(
...
private fun reduceSyncAndIntegration(
    ...
    val newState = when (action) {
        ...
        is GatekeeperAction.NotificationIntercepted -> {
            val log = NotificationLog(packageName = action.packageName, title = action.title, content = action.content, timestamp = action.timestamp)
        ...
        is GatekeeperAction.CancelScheduledMessage -> {
            ...
            slice.copy(scheduledMessages = slice.scheduledMessages.map { if (it.id == action.id) it.copy(status = MessageStatus.CANCELLED) else it })
        }
        is GatekeeperAction.MessageDelivered -> {
            ...
            slice.copy(scheduledMessages = slice.scheduledMessages.map { if (it.id == action.id) it.copy(status = MessageStatus.SENT) else it })
        }
        is GatekeeperAction.MessageFailed -> {
            ...
            slice.copy(scheduledMessages = slice.scheduledMessages.map { if (it.id == action.id) it.copy(status = MessageStatus.FAILED) else it })
        }
        else -> slice
    }
    return Update(newState, effects)
