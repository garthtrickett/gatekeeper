...
class AndroidEffectHandler(
    ...
    override fun scheduleBeeperMessage(message: ScheduledMessage, delayMillis: Long) {
    ...
    override suspend fun searchPodcasts(query: String): Either<String, List<PodcastFeedDto>> =
        podcastIndexClient.searchPodcasts(query)

    override suspend fun fetchPodcastFeed(url: String): Either<String, RssFeedData> =
        rssClient.fetchFeed(url)

    override suspend fun fetchUrlMetadata(url: String, isSoundCloud: Boolean, isGeneric: Boolean): Either<String, ContentMetadata> =
        urlMetadataClient.fetchMetadata(url, isSoundCloud, isGeneric).mapLeft { "Error fetching metadata" }

    override suspend fun syncBeeperChats(): Either<String, List<BeeperChat>> =
        beeperClient.getChats()
}