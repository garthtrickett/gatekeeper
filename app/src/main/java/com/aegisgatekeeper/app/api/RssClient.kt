...
object RssClient {
    suspend fun fetchFeed(feedUrl: String): Either<String, RssFeedData> = withContext(Dispatchers.IO) {
        try {
            ...
            val channelTitle = Regex("<title>(.*?)</title>").find(xml)?.groupValues?.get(1)?.replace("<!\\[CDATA\\[".toRegex(), "")?.replace("]]>".toRegex(), "")?.trim() ?: "Unknown Podcast"
            val imageMatch = Regex("<image>.*?<url>(.*?)</url>.*?</image>", RegexOption.DOT_MATCHES_ALL).find(xml)
            ...
            val episodes = items.mapNotNull { match ->
                val itemXml = match.value
                val title = Regex("<title>(.*?)</title>").find(itemXml)?.groupValues?.get(1)?.replace("<!\\[CDATA\\[".toRegex(), "")?.replace("]]>".toRegex(), "")?.trim()
