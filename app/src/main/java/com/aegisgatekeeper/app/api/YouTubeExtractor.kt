...
class YouTubeExtractor(private val client: HttpClient) {
        ...
        try {
            ...
            if (response.status.value in 200..299) {
                ...
                if (cobalt.url != null) {
                    ...
                    // Rewrite localhost if we are hit a local tunnel via ADB
            ...
            com.aegisgatekeeper.app.domain.platformLog("Gatekeeper", "⚠️ YouTubeExtractor: Attempt failed for $endpoint")
        }
        return null
    }
    ...
    suspend fun extractVideo(item: ContentItem): Either<String, ContentItem> {
        ...
        val endpoints = listOf(
            com.aegisgatekeeper.app.BuildConfig.COBALT_API_URL,
            "http://localhost:9000/",
            "http://127.0.0.1:9000/",
            "http://10.0.2.2:9000/",
            "http://localhost:9099/"
